...
export default function App() {
  ...
  const fetchAllMovies = async () => {
    ...
    setTrending(filterByGenre(trend, genre));
    setTopRated(filterByGenre(top, genre));
    setPopular(filterByGenre(pop, genre));
    setUpcoming(filterByGenre(upc, genre));
    setNowPlaying(filterByGenre(now, genre));
    setLoading(false);
  };
  ...
  const BookingModal = () => (
    <Modal visible={showBookings} animationType="slide">
      <View style={styles.bookingContainer}>
        ...
        <TouchableOpacity style={styles.button} onPress={() => setShowBookings(false)}>
  ...
  return (
    <View style={styles.container}>
      ...
      <Picker selectedValue={genre} style={styles.picker} onValueChange={(itemValue) => setGenre(itemValue)}>
        {genres.map((g) => (
          <Picker.Item key={g.value} label={g.label} value={g.value} />
        ))}
      </Picker>

      <TouchableOpacity style={[styles.button, { marginHorizontal: 20 }]} onPress={() => setShowBookings(true)}>
...
const styles = StyleSheet.create({
  ...
  picker: { color: '#fff', marginHorizontal: 20, marginBottom: 15, backgroundColor: '#1c1c1c', fontSize: 18 },
  button: { backgroundColor: '#00bfff', padding: 12, borderRadius: 10, alignItems: 'center', marginVertical: 10 },
  buttonText: { color: '#fff', fontWeight: 'bold', fontSize: 16 },
  loading: { flex: 1, justifyContent: 'center', alignItems: 'center', backgroundColor: '#121212' },
  bookingContainer: { flex: 1, backgroundColor: '#121212', padding: 20, alignItems: 'center' },
  bookingItem: { padding: 10, marginVertical: 5, backgroundColor: '#1c1c1c', borderRadius: 8, width: '100%' },
});
